"use client";
import LoginUI from "@/app/auth/login/page";
import LogoutButton from "@/app/components/logout/page";
import React, { useEffect, useState } from "react";
import Cookies from "js-cookie";
import useStore from "@/app/store/page";
import useSWR from "swr";
import { GET_CART } from "@/app/Api/api_list";
import LogoButton from "@/app/common/logo/page";

const Navbar = () => {
  const [loginstatus, setloginstatus] = useState(false);
  const [userId, setUserId] = useState(Cookies.get("_ae_user_id"));

  const closemodal = (close: any) => {
    setloginstatus(close);
  };

  const onLogout = () => {
    Cookies.remove("_ae_user_id");
    setUserId(""); //
  };

  useEffect(() => {
    setUserId(Cookies.get("_ae_user_id"));
  }, [userId]);
  const getuserid = Cookies.get("_ae_user_id");
  const cartid = Number(Cookies.get("_ae_cart_id"));

  const fetcher = (url: any) => fetch(url).then((res) => res.json());
  const { data, error } = useSWR(`${GET_CART}/${cartid}`, fetcher);
  ...
  return (
    <div>
                ...
                <li className="mb-6 hidden max-lg:block">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      className="px-4 py-2 text-sm rounded-full text-white border-2 border-[#007bff] bg-[#007bff] hover:bg-[#004bff]"
                    >
                      Sign In
                    </button>
                  </div>
      ...
      {loginstatus && <LoginUI closemodal={closemodal} />}
    </div>
  );
};
